...
async function showProductDetails(productId) {
    if (!client) {
        ...
        console.log(chalk_1.default.dim('Run: chariow config --set <your_api_key>\n'));
        return;
    }
    ...
    try {
        ...
        if (product.bundle) {
            console.log(chalk_1.default.yellow('\n📦 BUNDLE INFORMATION\n'));
            const bundleTable = new cli_table3_1.default({
                colWidths: [20, 30],
                style: { 'padding-left': 1, 'padding-right': 1 }
            });
            bundleTable.push([chalk_1.default.gray('Bundle Value:'), product.bundle.value?.formatted || 'N/A']);
            bundleTable.push([chalk_1.default.gray('Savings:'), `${product.bundle.savings?.percentage || '0%'} (${product.bundle.savings?.amount?.formatted || '$0'})`]);
...
async function showTopProducts() {
    const spinner = (0, ora_1.default)('Loading top products...').start();
    try {
        if (client) {
