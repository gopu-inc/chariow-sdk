...
// Fonction utilitaire pour padder sans erreur
...
export async function interactiveMode() {
  ...
  // Extraire le prix numérique d'un produit
  ...
  const getSalesCount = (product: any): number => {
    ...
    if (product.sales_count && typeof product.sales_count === 'object') {
      // Si c'est un objet, essayer d'extraire une valeur
      return (product.sales_count as any).value || (product.sales_count as any).count || 0;
    }
    return 0;
  };
  
  // Calculer le revenu total correctement
  const getTotalRevenue = (): number => {
    return products.reduce((sum, p) => {
      const price = getPriceValue(p);
      const sales = getSalesCount(p);
      // Le revenu = prix * nombre de ventes
...
`));
    ...
    console.log(colors.border('├──────────────────────────────────────────────────────────────────┤'));
    const actionsLine = `  ${icons.up}${icons.down} Navigate    ${icons.enter} View Details    ${icons.refresh} Refresh    ${icons.config} Config    ${icons.explore} Explore    ${icons.quit} Quit`;
    ...
    console.log(colors.text(`\n  ${statusIcon} ${statusText}  │  Last sync: ${timeStr}  │  Chariow CLI v2.0\n`));
  };
